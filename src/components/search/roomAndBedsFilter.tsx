...
export default function RoomAndBedsFilter({
  ...
  useEffect(() => {
    ...
    if (collectedProperties.bedrooms && collectedProperties.bedrooms.length) {
      setMaxBedrooms(Math.max(...collectedProperties.bedrooms));
    }

    if (collectedProperties.beds && collectedProperties.beds.length) {
      setMaxBed(Math.max(...collectedProperties.beds));
    }

    if (collectedProperties.bathrooms && collectedProperties.bathrooms.length) {
  ...
  return (
    <div className={"grid grid-cols-2 gap-2"}>
      ...
      <div className={"flex gap-2 items-center justify-end"}>
        ...
        />
        <h4 className={"font-normal mb-0 pb-0 w-6 text-center"}>
        ...
        <Button
          ...
          icon={<PlusOutlined />}
          shape={"circle"}
          disabled={bedrooms === maxBedrooms}
          onClick={() =>
            setBedrooms((prev: number) => {
              if (prev === maxBedrooms) {
                return prev;
              } else {
                return prev + 1;
      ...
      <div className={"flex gap-2 items-center justify-end"}>
        ...
        <Button
          ...
          icon={<PlusOutlined />}
          shape={"circle"}
          disabled={bathrooms === maxBath}
          onClick={() =>
            setBathrooms((prev: number) => {
              if (prev === maxBath) {
                return prev;
              } else {
                return prev + 1;
