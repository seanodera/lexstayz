                ...
                >
                    {userDetails?.paymentMethods?.map((paymentMethod: PaymentMethod, index: number) => (
                        <Radio key={index} value={paymentMethod.authorization_code} className="">
                            {({ checked }) => (
                                <div className={`w-full ${checked ? 'border-primary ' : 'border-gray-400'} flex gap-2 border-solid py-3 px-2 rounded-xl `}>
                                    <div className={'text-3xl'}>
                                        {paymentMethod.card_type === 'visa'? <SiVisa /> : paymentMethod.card_type === 'mastercard'? <SiMastercard/> : <CiCreditCard2/>}
                                    </div>
                                <div >
