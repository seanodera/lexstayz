  ...
  async (
    ...
    try {
      ...
      const payload = await dispatch(
        getUserDetailsAsync(userCredential.user.uid)
      );
      console.log(payload);
...
export const deleteFromWishList = createAsyncThunk(
  "authentication/deleteFromWishlist",
  async ({ stayId }: { stayId: string }, { getState, rejectWithValue }) => {
    const { authentication } = getState() as RootState;
    try {
      const user = authentication.user;
  ...
  async (
    ...
    try {
      ...
      console.log({
        ...
        country: country
          ? country
          : { currency: "GHS", emoji: "GH", name: "Ghana" },
      });
      return {
        data: data,
        country: country
